...
export default function SongReveal({
  ...
  useEffect(() => {
    setReviewSong(song)
    if (!song?.providerTrackId || (song.title && song.artist && song.album && song.artwork && (song.externalUrl || song.external_urls?.spotify))) return undefined
    let alive = true
  ...
  const spotifyTrackId = reviewSong.providerTrackId || String(reviewSong.id || '').replace(/^spotify:/, '')
  const spotifyUrl = reviewSong.externalUrl || reviewSong.external_urls?.spotify || (spotifyTrackId ? `https://open.spotify.com/track/${encodeURIComponent(spotifyTrackId)}` : '')
    ...
    <div className="song-reveal">
      ...
      <div className="song-reveal__card">
        ...
        <div className="song-reveal__info">
          ...
          <p className="song-reveal__guess">YOUR GUESS: ${userGuess || 'No guess submitted'}</p>
          ...
          ${spotifyUrl && html`<a className="song-reveal__details-btn" href=${spotifyUrl} target="_blank" rel="noreferrer">OPEN IN SPOTIFY</a>`}
