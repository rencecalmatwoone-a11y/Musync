...
export async function resolveVSAudio(track) {
  ...
  if (!resolved && track.spotifyPreviewUrl) {
    resolved = { provider: 'spotify-preview', previewUrl: track.spotifyPreviewUrl, duration: Number(track.durationMs) || 30000 }
  }
...
export async function searchVSAudioTracks(genre = 'Any Genre', limit = 30) {
  ...
  const unique = [...new Map(responses.flat()
